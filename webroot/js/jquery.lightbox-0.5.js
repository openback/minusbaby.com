		...
		function _start(objClicked,jQueryMatchedObj) {
			...
			// We have an image set? Or just an image? Let´s see it.
                        if ( jQueryMatchedObj.length == 1 ) {
                            if (jQueryMatchedObj[0].getAttribute('rev')) {
				...
				for ( var i = 0; i < jQueryMatchedObj.length; i++ ) {
                                    if (jQueryMatchedObj[i].getAttribute('rev')) {
		...
		function _set_interface() {
			...
			$('#lightbox-loading-link,#lightbox-secNav-btnClose').click(function() {
				...
				return false;
                        ...
                        // Assign the external link for the image
                        $('#lightbox-nav').click(function() {
                                if (settings.imageArray[settings.activeImage].length >= 3) {
                                    window.open(settings.imageArray[settings.activeImage][2]);
                                } else {
                                    return false;
                                }
                            });
